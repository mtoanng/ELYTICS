...
def make_soh_colored_subplot(template, sample_name, xaxis_col, color_label_name):
    ...
    fig.update_layout(
        template=template,
        showlegend=True,
        title=dict(text=f"{sample_name}: Stack SOH colored by {color_label_name}", x=0.5, xanchor="center"),
...
def create_polcurve_decomp_plot(dff, valid_ivs, slider_value, plotly_template, sample_name):
    ...
    fig_soh_split.update_layout(
        template=plotly_template,
        title=dict(text=f"{sample_name}: Pol Curve Change (@Ref OpCons) Related to Ageing", x=0.5, xanchor="center"),
...
def create_load_cycle_plots(dff, theme_data, sample_name, slider_value, valid_ivs):
    ...
    fig.update_layout(
        template=plotly_template,
        title=dict(
            text=f"{title_prefix}Load Cycle Histograms (IV {int(iv_0)} @ {rt_0:.0f}h → IV {int(iv_1)} @ {rt_1:.0f}h)",
