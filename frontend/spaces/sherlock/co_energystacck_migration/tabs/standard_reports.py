...
* On-the-fly silver data loading with automatic unload of the previous series.
...
from ..backend.data_visualization import plot_report
from ..backend.paths import STANDARD_REPORTS_PATH
...
def register_callbacks(app, data_manager: SeriesDataManager):
    ...
    def on_modal_save(n, filename, filetype, pending_data):
        ...
        try:
            ...
            res_lbl = resolution_label(
                current_resolution or initial_resolution(agg_levels))
            report_name = (STANDARD_REPORTS[report_idx]["name"]
                           if report_idx is not None else "")

            fig, img_title = prepare_figure_for_image_export(
                figure_dict, series_name, res_lbl, report_name)

            if filetype == "pdf":
                png_bytes = figure_to_image_bytes(fig, fmt="png")
                pdf_bytes = png_to_pdf_bytes(png_bytes, title=img_title)
                pdf_b64 = base64.b64encode(pdf_bytes).decode()
                return dict(content=pdf_b64, filename=full_filename, base64=True), False, f"Saved PDF -> {full_filename}"
            else:
                fmt = "jpeg" if filetype == "jpg" else filetype
                img_bytes = figure_to_image_bytes(fig, fmt=fmt)
                img_b64 = base64.b64encode(img_bytes).decode()
                return dict(content=img_b64, filename=full_filename, base64=True), False, f"Saved image -> {full_filename}"
